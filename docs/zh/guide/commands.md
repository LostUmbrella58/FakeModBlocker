...
| `/modblocker violations <玩家>` | 查看已记录的违规计数（见[阶梯处罚](./escalation)），离线玩家同样可查 |
| `/modblocker clear <玩家> [模组]` | 清空该玩家的计数：不填模组则全清，填了只清那一个 |
| `/modblocker discord test` | 经由 [DiscordSRV](./discord) 发一条测试消息 |
...
| 权限节点 | 默认 | 说明 |
|----------|------|------|
| `fakemodblocker.admin` | op | 使用管理命令（reload / check / violations / clear / discord） |
